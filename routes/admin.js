...
Router.route("/User").get(async (req, res) => {
    ...
    const {data: {success, users}} = await axios.get("https://arial-movie.herokuapp.com/api/v1/user?userOnly=1", {headers: {Authorization: "Bearer " + req.cookies.accessToken}})
...
Router.route("/Comment").get(async (req, res) => {
    const {data: {movies}} = await axios.get("https://arial-movie.herokuapp.com/api/v1/movie?active=trailer,poster")
    res.render('ManageComment', {movies, user: req.user})
})

Router.route("/Comment/:movieId").get(async (req, res) => {
    const { data: { comments, success: successCmt } } = await axios.get(`https://arial-movie.herokuapp.com/api/v1/comment?movieId=${req.params.movieId}&sort=-createdAt`)
    if (!successCmt) return res.status(StatusCodes.BAD_REQUEST).json({ success: false })
    
    const userId = comments.map(item => item.createdBy)
    const { data: { users, success } } = await axios.post(`https://arial-movie.herokuapp.com/api/v1/user/getUsers`, { userId }, {headers: {Authorization: "Bearer " + req.cookies.accessToken}})
    if (!success) return res.status(StatusCodes.BAD_REQUEST).json({ success: false })

    const { data: { movie, success: successMovie } } = await axios.get(`https://arial-movie.herokuapp.com/api/v1/movie/${req.params.movieId}`)
